  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-xl shadow p-4">
        <h3 className="text-lg font-bold mb-4 text-[#4300FF]">All Banners</h3>
        {loading ? (
          <div className="text-gray-500">Loading...</div>
        ) : banners.length === 0 ? (
          <div className="text-gray-500">No banners yet.</div>
        ) : (
          <ul className="space-y-3">
            {banners.map(b => (
              <li key={b.id} className="flex justify-between items-center border-b pb-2 gap-4">
                <div className="flex items-center gap-3">
                  {b.imageUrl ? (
                    <img src={b.imageUrl} alt="Banner" className="w-20 h-12 object-cover rounded" />
                  ) : null}
                </div>
                <button onClick={() => handleDelete(b.id)} className="text-red-500 hover:underline text-sm">Delete</button>
              </li>
            ))}
          </ul>
        )}
